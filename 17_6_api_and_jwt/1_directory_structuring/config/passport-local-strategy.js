    ...
    function(req,email,password,done){
        ...
        User.findOne({email:email},function(err,user){
            ...
            if(!user || user.password != password){
                req.flash('error','Envalid username/password');
...
passport.checkAuthentication = function(req,res,next){  // own written funtion inside passport
...
passport.setAuthenticatedUser = function(req,res,next){
    if(req.isAuthenticated()){
        //req.user contain the current signed in user from session cookie and
